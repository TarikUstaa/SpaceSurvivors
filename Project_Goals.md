...
| # | Goal | Notes |
|---|------|-------|
| G1 | **Swarm mechanic** | ✅ (2026-09-01, awaiting sign-off) `Environment/SwarmEvent` — a `SpaceEventBehaviour` that pours enemies in from 1–2 screen edges. Runs on `EventDirector`'s **own fixed track** (`_swarmEvent` / `_swarmInterval` **45s**, first at 60s), independent of and able to overlap the random event rotation; removed from the random catalogue. Real enemies (loot/XP/alive-count, not auto-released). Uses `EventBanner`. Roster: weighted Grunt/Swarmer + Charger/Shooter. **v2 (`dba5f81`): size scales with run time** — `30 + 9×minutes`, cap 150 (was a flat 34 that a mid-game build shredded instantly); duration 16→20s. In the sim it's now the heartbeat of danger in the first ~3 min (HP oscillates 65–115 instead of pinned at max). |
...
| G3 | **Skill / upgrade balance pass** | ✅ (2026-09-01, awaiting sign-off) All 16 level-up choices retuned in one place — `Editor/BalanceConfig.TuneUpgrades()` now owns weight / per-stack value / stack ceiling for every upgrade (stack ceilings on evolution catalysts double as "picks to evolve"). Headline moves: **FireRate** 6→5 stacks / +20→18% (was the runaway passive — multiplies weapon + mine cadence); **MoveSpeed** 6→4 / +12→10% (kiting snowball + the Mine evo was gated at an unreachable 6); **Haste** w0.9→0.5, 5→3 (projectile speed is a near-dead stat only wanted as the Void Lance catalyst); **MaxHealth** +25→30/stack, 6→5 (G4's swarms hit harder); **MultiShot** w0.35→0.5 (almost never offered); **Damage** 4→5. Weapon-grant cards nudged just below passives. Plus a **weapon slot cap** — `WeaponController._maxWeapons = 6` (evolutions swap in place, don't count); `UpgradeService.Roll` stops offering new weapons at cap. Late-run builds are now about a loadout, not collecting every gun. |
| G4 | **More enemies on screen + threat** | 🟡 (2026-09-01, iter-6b `dba5f81` — early game fixed, mid/late needs another pass) A sim playtest of iter-5 found the run had **no threat after ~90s**: a kiting bot never dropped below ~90% HP in 8 min and the screen sat at 10–25 enemies (player DPS outran the spawn curve; enemies far slower than the player; random spawn placement left them behind a kiter to far-cull). iter-6b: **`SpawnDirector` lead-biased placement** (60% of spawns in a 75° arc toward the player's heading, `_leadBias`/`_leadArcDegrees`) so the horde stays in play; **enemy base speeds up** (Grunt 2.2→2.6, Swarmer 3.1→4.0, Shooter 2.4→2.7, Charger 2.0→2.3, Splitter 1.9→2.1, Brute 1.1→1.25); **speed & health multipliers ramp harder and further** (HP ×4 by 10 min, ×11.5 by 25 min infinite); spawn curve first ~90s pulled *below* iter-5 (lead-spawn hits harder) then ramps past it; `maxAliveEnemies` 550/480; enemy prefab `_farCullRadius` 45→55. Verified: first ~3 min now genuinely dangerous. **Still open:** past ~3 min a snowballed build trivialises the lulls between swarms — iter-6c (steeper mid/late curves + swarm `9→13`/min) is drafted, needs a longer sim / the human playthrough to calibrate. |
